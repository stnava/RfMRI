...
\begin{itemize}[<+->]
\item High-level script {\em univar\_multivar\_fmri\_consistency.sh} runs all data through to get
...
if ( file.exists( fn1 ) &  file.exists( fn2 ) & file.exists(temfn) )
  ...
  myth<-paste("0.0001x",as.character(max(i1[ i1 > 0 ])  ) )
