...
\draw[->] (fmri) to [out = 0, in = 180, looseness = 1] (sltim);
 ...
 \draw[->] (nuis) to [out = 0, in = 180, looseness = 1] (hipass);
 \draw[->] (hipass) to [out = 0, in = 180, looseness = 1] (mstats);
 \draw[->] (hipass) to [out = 0, in = 180, looseness = 1] (ac);
 \draw[->] (ac) to [out = 0, in = 180, looseness = 1] (ustats);
...
\item Multi/Univariate use same pre-processing and includes high-pass filtering.
\end{itemize}
\end{frame}
...
\begin{itemize}[<+->]
\item Use test-retest data as training-testing data
\item Threshold statistical map consistently:  top x\% of $\beta$ map
\item Signal correlation / difference across runs
\item Peak activation
\item Signal overlap (Dice) between clusters in thresholded beta map between two
  runs  
\item Minimum distance sum (MDS) between clusters in thresholded beta map between two
  runs  
 \begin{block}{Prediction}
 Significance of training features in testing run 
 \end{block}
\end{itemize}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Analysis Scripts}
in RfMRI scripts directory 
\begin{itemize}[<+->]
\item process.sh --- calls scripts for each subject (should write your
  own according to data organization)
...
10 subjects 
\begin{itemize}[<+->]
\item Do $\beta$ maps overlap?  somewhat
\item Do $\beta$ maps correlate?  {\em Yes} Over 0.83
...
fn<-Sys.glob("../../*/*/*run001/maxbeta.csv")
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Univariate Overlap Distribution}
<<out.width='.5\\linewidth',dev='pdf',results='show',eval=TRUE,warning=FALSE,echo=FALSE>>= 
fn<-Sys.glob("../../*/*/compareu/*y_eval.csv")
print(paste(length(fn),"subjects"))
diceu<-rep(NA,length(fn))
for ( i in 1:length(fn) )  diceu[i]<-read.csv(fn[i])$Dice
################
qdata <- data.frame(diceu)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Univariate Significance in Test Data: Raw Data}
<<out.width='.8\\linewidth',dev='pdf',results='show',eval=TRUE,warning=FALSE,echo=FALSE>>= 
fn<-Sys.glob("../../*/*/compareu/prediction_x.csv")
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}[fragile]
\frametitle{Multivariate Significance in Test Data: Raw Data}
<<out.width='.8\\linewidth',dev='pdf',results='show',eval=TRUE,warning=FALSE,echo=FALSE>>= 
fn<-Sys.glob("../../*/*/comparem/prediction_x.csv")
for ( i in 1:length(fn) ) print(data.frame(read.csv(fn[i],h=T)[,c(1,3,5)]))
@
...
\begin{itemize}[<+->]
\item Parameter exploration study for both univariate and multivariate
  approaches
\item How do smoothing and specificity interact?
\item Autocorrelation correction may be useful 
